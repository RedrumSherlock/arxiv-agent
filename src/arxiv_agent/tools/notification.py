...
def send_status_email(
    ...
    """
    Send a status notification via email (e.g., when no papers found or errors occurred).
    ...
    payload = {
        "sender": {"email": sender_email, "name": sender_name},
        "to": [{"email": email} for email in email_list],
        "subject": "Arxiv Agent Status Update",
        "htmlContent": html_content,
    }
...
def send_status_webhook(
    ...
    """
    Send a status notification via webhook (e.g., when no papers found or errors occurred).
    ...
    if is_google_chat:
        payload = {"text": f"📚 *Arxiv Agent Status*\n\n{message}"}
    else:
        payload = {
            "type": "arxiv_status",
            "message": message,
    ...
    try:
        ...
        logger.error(f"Failed to send status webhook: {e}")
        return NotificationResult(success=False, channel="webhook", message=str(e))
